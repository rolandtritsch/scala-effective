...
class HeapProperties(heapInterface: HeapInterface) extends Properties("Heap"):
  ...
  property(
    "inserting the minimal element and then finding it should"
      ++ " return the same minimal element"
  ...
  property(
    "the minimum of a heap of two elements should"
      ++ " be the smallest of the two elements"
  ...
  property(
    "delete minumum of heap of one element should"
      ++ " return an empty heap"
  ) = forAll { (x: Int) =>
    val heap1: List[Node] = insert(x, List.empty[Node])
