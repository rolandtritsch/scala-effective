...
/** This file contains the definition of the interface for the `Heap` type, and
...
/** Interface for the heap data structure
  *
  * This interface defines the operations for constructing and manipulating a
  * heap. A heap is modeled by the type `List[Node]`, and the only way to
  * construct a heap is to use the constructor `empty`, and then to use the
  * operation `insert` to add more elements to it.
  *
  * @see
...
trait HeapInterface:
  ...
  /** Merge two heaps
    *
    * @return
    *   The new/updated/merged heap
    */
  def meld(heap1: List[Node], heap2: List[Node]): List[Node]

  /** Find the minimum value of a heap, and/or raise an error if the heap is
    * empty
    *
    * @return
    *   The minimum value
...
/** The remaining of the file contains correct and bogus implementations of the
  * heap interface.
  *
...
class BinomialHeap extends HeapInterface:
  ...
  protected def link(node1: Node, node2: Node): Node =
    require(node1.rank == node2.rank, "node1.rank == node2.rank")
    ...
    (heap1, heap2) match
      ...
      case (node1 :: heap1, node2 :: heap2) =>
        if node1.rank < node2.rank then
          node1 :: meld(heap1, node2 :: heap2)
        else if node2.rank < node1.rank then
          node2 :: meld(node1 :: heap1, heap2)
        else
          ins(link(node1, node2), meld(heap1, heap2))
    ...
    case node :: heap =>
      def getMin(node: Node, heap: List[Node]): (Node, List[Node]) = heap match
        ...
        case node2 :: heap2 =>
          ...
          if root(node) <= root(node3) then (node, heap)
