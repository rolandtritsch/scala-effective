...
class QuickCheckSuite extends munit.FunSuite:
  def checkBogus(heapInterface: HeapInterface): Unit =
    ...
    assert(
      check(asProp(HeapProperties(
        new quickcheck.test.BinomialHeap {}
      )))(identity).passed,
      "The properties were not satisfied by the correct binomial heap"
        ++ " implementation. Make sure the properties you write are true"
        ++ " of heaps."
    )
  ...
  def asProp(properties: Properties): Prop =
    Prop.all(properties.properties.map(_(1)).toSeq*)
