...
class TodoSuite extends FunSuite:
  ...
  def makeModelFixture(name: String, model: Model) =
    FunFixture[(String, Model)](
      setup = { test => model.clear(); (name, model) },
  ...
  // Custom assertions. These make our code more readable and give more
  // useful messages on errors
  ...
  def assertTask(
    ...
    actual match
      case None =>
        fail(
          s"Using $modelName: We expected the task $expected but we received None instead."
        )
      case Some(task) =>
        assertEquals(
          task,
  ...
  def assertTaskActive(
    ...
    task match
      case None =>
        fail(
          s"Using $modelName: We expected a task but we received None instead."
        )
      case Some(t) =>
        assert(
          t.state.active,
  ...
  def allTests(fixture: FunFixture[(String, Model)]): Unit =
    ...
    fixture.test("Deleted tasks are no longer read") { case (name, model) =>
      ...
      assert(
        model.delete(id1),
        s"Using $name: We expected deleting $id1 to return true"
      )
    ...
    fixture.test("Tasks(tag) returns only tasks with given tag") {
      case (name, model) =>
        ...
        assertEquals(
          model.tasks(Tag("a")).toList,
          List(id1 -> task1, id3 -> task3)
        )
        assertEquals(model.tasks(Tag("b")).toList, List(id1 -> task1))
        assertEquals(
          model.tasks(Tag("c")).toList,
          List(id2 -> task2, id3 -> task3)
        )
    }

    fixture.test(
      "complete changes state to completed, if task is not already completed"
    ) { case (name, model) =>
      val id1 = model.create(task1)
      val id2 = model.create(task2)
