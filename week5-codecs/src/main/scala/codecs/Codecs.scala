...
/** A data type modeling JSON values.
  ...
  */
sealed trait Json:
  /** Try to decode this JSON value into a value of type `A` by using the given
    * decoder.
    *
...
trait DecoderInstances:
  ...
  given listDecoder[A](using decoder: Decoder[A]): Decoder[List[A]] =
    Decoder.fromFunction {
      case Json.Arr(as) => as.foldLeft(Some(List.empty[A]): Option[List[A]]) {
        (list, a) => list match
          case Some(l) => decoder.decode(a) match
            case Some(e) => Some(l :+ e)
            case None => None
          case None => None
      }
      case _ => None
  ...
  /** A decoder for JSON objects. It decodes the value of a field of the
    * supplied `name` using the given `decode */
  def field[A](name: String)(using decoder: Decoder[A]): Decoder[A] =
    Decoder.fromFunction {
      case Json.Obj(fs) => decoder.decode(fs.get(name).get)
      case _ => None 
    }
